...
/** Interactive Site Supervisor job-role marketplace built from the supplied design. */
public class SiteSupervisorJobRolesPage {
    ...
    private Node header(Runnable backAction) {
        ...
        HBox brand = new HBox(12, logo, label("Supervisor Job Roles", "-fx-font-family:Georgia;-fx-font-size:26px;-fx-font-weight:700;-fx-text-fill:#574500;"));
        ...
        worker.setOnAction(e -> { if (backAction != null) backAction.run(); });
        ...
        header.setLeft(brand); header.setCenter(navigation); header.setRight(new HBox(12, login, signUp));
    ...
    private Node hero() {
        ...
        slideStatus.setStyle("-fx-background-color:rgba(30,27,21,.68);-fx-background-radius:14px;-fx-text-fill:#ffffff;-fx-font-size:12px;-fx-font-weight:700;-fx-padding:6px 11px;");
        ...
        updateSlideStatus();
        startSlider();
        Label quote = label("\"Great structures rise not just from bricks and mortar, but from the clear vision, steady guidance, and unwavering dedication of a skilled supervisor.\"",
                "-fx-font-family:Georgia;-fx-font-size:22px;-fx-font-style:italic;-fx-text-fill:#4d4635;-fx-line-spacing:6px;");
        quote.setWrapText(true); quote.setMaxWidth(500);
        VBox words = new VBox(quote);
        words.setAlignment(Pos.CENTER_LEFT); words.setPrefHeight(285); words.setPadding(new Insets(12, 0, 12, 26));
        words.setStyle("-fx-border-color:#d4af37;-fx-border-width:0 0 0 4px;");
        HBox hero = new HBox(54, visual, words); hero.setAlignment(Pos.CENTER);
        return hero;
    }

    private Node filterBar() {
        state = choice(ALL, "Maharashtra", "Karnataka", "Odisha", "Haryana", "Uttar Pradesh", "Kerala", "Gujarat", "Madhya Pradesh");
        city = choice(ALL, "Pune", "Mumbai", "Bangalore", "Gurgaon", "Noida", "Panvel", "Jalna", "Indore");
        role = choice(ALL, "Site", "Construction", "Electrical", "Safety", "Masonry", "Plumbing", "Carpenter", "Labour");
        Button clear = outline("Clear filters");
        clear.setOnAction(e -> { state.setValue(ALL); city.setValue(ALL); role.setValue(ALL); showMatches(); });
        Button find = primary("Find roles"); find.setOnAction(e -> showMatches());
        HBox controls = new HBox(12, filterField("STATE", state), filterField("CITY", city), filterField("ROLE", role), clear, find); controls.setAlignment(Pos.BOTTOM_CENTER);
        VBox bar = new VBox(15, label("Find a suitable job role for you", "-fx-font-size:20px;-fx-font-weight:700;-fx-text-fill:#1e1b15;"), controls);
        bar.setAlignment(Pos.CENTER); bar.setPadding(new Insets(24)); bar.setStyle(cardStyle("#ffffff"));
        return bar;
    }

    private Node opportunities() {
        resultText.setStyle("-fx-font-size:14px;-fx-text-fill:#4d4635;");
        Label eyebrow = label("CURATED FOR YOU", "-fx-font-size:11px;-fx-font-weight:800;-fx-text-fill:#735c00;-fx-letter-spacing:1px;");
        VBox heading = new VBox(5, eyebrow, label("Available opportunities", "-fx-font-family:Georgia;-fx-font-size:32px;-fx-font-weight:700;-fx-text-fill:#1e1b15;"), resultText);
        cards.setAlignment(Pos.CENTER); cards.setPrefWrapLength(1170);
        showMatches();
    ...
    private void showMatches() {
        ...
        int count = 0;
        for (String[] job : JOBS) if (matches(job)) { cards.getChildren().add(card(job)); count++; }
        resultText.setText(count == 0 ? "No roles found. Try clearing one or more filters." : count + " role" + (count == 1 ? "" : "s") + " available");
    }
    ...
    private boolean matches(String[] job) {
        return (ALL.equals(state.getValue()) || job[3].equalsIgnoreCase(state.getValue()))
                ...
                && (ALL.equals(role.getValue()) || job[0].toLowerCase(Locale.ROOT).contains(role.getValue().toLowerCase(Locale.ROOT)));
    }
    ...
    private Node card(String[] job) {
        ...
        Label verified = label("VERIFIED OPPORTUNITY", "-fx-background-color:#f6e7ae;-fx-background-radius:10px;-fx-text-fill:#574500;-fx-font-size:10px;-fx-font-weight:800;-fx-padding:4px 8px;");
        Label place = label("Location: " + job[2] + ", " + job[3].toUpperCase(Locale.ROOT), "-fx-font-size:13px;-fx-text-fill:#4d4635;");
        Label wage = label("Daily wage: ₹" + job[4], "-fx-font-size:15px;-fx-font-weight:700;-fx-text-fill:#574500;");
        Button apply = primary("APPLY"); apply.setMaxWidth(Double.MAX_VALUE);
        apply.setOnAction(e -> applyForRole(job, apply));
        apply.setOnAction(e -> { apply.setText("APPLIED ✓"); apply.setDisable(true); apply.setStyle("-fx-background-color:#685c52;-fx-background-radius:10px;-fx-text-fill:white;-fx-font-size:13px;-fx-font-weight:800;-fx-padding:11px 22px;"); });
        VBox details = new VBox(8, verified, name, hindi, line(), place, wage, apply);
        details.setPadding(new Insets(17)); details.setAlignment(Pos.TOP_LEFT);
        VBox card = new VBox(photo, details); card.setPrefSize(336, 415); card.setStyle(cardStyle("#ffffff"));
    ...
    private Node bottomBar(Runnable backAction) {
        Button back = outline("← Back to skills"); back.setOnAction(e -> { if (backAction != null) backAction.run(); });
        HBox bar = new HBox(back); bar.setAlignment(Pos.CENTER_LEFT); bar.setPadding(new Insets(13, 58, 13, 58));
    ...
    private ComboBox<String> choice(String... values) {
        ...
        box.setValue(ALL); box.setPrefWidth(180);
        box.setStyle("-fx-background-color:#faf3e8;-fx-border-color:#7e7665;-fx-border-radius:10px;-fx-background-radius:10px;-fx-font-size:13px;-fx-padding:3px 8px;");
        return box;
    }

    private VBox filterField(String title, ComboBox<String> field) {
        Label caption = label(title, "-fx-font-size:10px;-fx-font-weight:800;-fx-text-fill:#685c52;-fx-letter-spacing:1px;");
    ...
    private void applyForRole(String[] job, Button apply) {
        ...
        apply.setStyle("-fx-background-color:#685c52;-fx-background-radius:10px;-fx-text-fill:white;-fx-font-size:13px;-fx-font-weight:800;-fx-padding:11px 22px;");
    ...
    private Rectangle roundClip(double width, double height) { Rectangle clip = new Rectangle(width, height); clip.setArcWidth(28); clip.setArcHeight(28); return clip; }
    private Node line() { StackPane line = new StackPane(); line.setPrefHeight(1); line.setMaxWidth(Double.MAX_VALUE); line.setStyle("-fx-background-color:#e5ded2;"); return line; }
    ...
    private Button nav(String text, boolean active) { Button button = new Button(text); button.setStyle("-fx-background-color:transparent;-fx-font-size:13px;-fx-font-weight:700;-fx-text-fill:" + (active ? "#735c00" : "#4d4635") + ";-fx-border-color:" + (active ? "#735c00" : "transparent") + ";-fx-border-width:0 0 2px 0;-fx-padding:8px 4px;-fx-cursor:hand;"); return button; }
    private Button primary(String text) { Button button = new Button(text); button.setStyle("-fx-background-color:#735c00;-fx-background-radius:10px;-fx-text-fill:white;-fx-font-size:13px;-fx-font-weight:800;-fx-padding:11px 22px;-fx-cursor:hand;"); return button; }
    private Button outline(String text) { Button button = new Button(text); button.setStyle("-fx-background-color:transparent;-fx-background-radius:10px;-fx-border-color:#7e7665;-fx-border-radius:10px;-fx-text-fill:#1e1b15;-fx-font-size:13px;-fx-font-weight:700;-fx-padding:10px 20px;-fx-cursor:hand;"); return button; }
    private String cardStyle(String background) { return "-fx-background-color:" + background + ";-fx-background-radius:14px;-fx-border-color:#d0c5af;-fx-border-radius:14px;-fx-effect:dropshadow(gaussian,rgba(58,48,39,.10),16,0,0,5px);"; }
}
