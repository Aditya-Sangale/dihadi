...
public class WorkerPage extends Application {
    ...
    public Scene getWorkerScene(Runnable backAction, Runnable aboutAction) {
        ...
        StackPane photo = new StackPane(heroImage); photo.setPrefSize(660, 372);
        startHeroSlider();
        photo.setStyle("-fx-background-color: #fbf3e5; -fx-background-radius: 24px; -fx-border-color: rgba(115,92,0,0.24); -fx-border-radius: 24px; -fx-effect: dropshadow(gaussian, rgba(58,48,39,0.16), 24, 0, 0, 8px);");
        HBox hero = new HBox(24, photo, copy); hero.setAlignment(Pos.CENTER); hero.setPadding(new Insets(30, 30, 30, 30)); hero.setMaxWidth(1180);
        hero.setStyle("-fx-background-color: rgba(255,248,240,0.94); -fx-background-radius: 24px; -fx-border-color: rgba(115,92,0,0.18); -fx-border-radius: 24px;");

        Label categoriesTitle = label("Find roles that match your mastery", "-fx-font-size: 30px; -fx-font-weight: 800; -fx-text-fill: #3a3027;");
        Label categoriesText = label("Select a worker category to view its available roles.", "-fx-font-size: 15px; -fx-text-fill: #3c3c3c;");
        FlowPane grid = new FlowPane(20, 20); grid.setAlignment(Pos.CENTER); grid.setMaxWidth(1160);
        for (int i = 0; i < NAMES.length; i++) grid.getChildren().add(categoryCard(i));
        VBox categories = new VBox(10, categoriesTitle, categoriesText, grid); categories.setAlignment(Pos.CENTER); categories.setPadding(new Insets(40, 32, 44, 32)); categories.setMaxWidth(1220);
        categories.setStyle("-fx-background-color: rgba(251,243,229,0.96); -fx-background-radius: 24px; -fx-border-color: rgba(115,92,0,0.18); -fx-border-radius: 24px;");

        VBox footer = createDesktopFooter();
        VBox content = new VBox(24, hero, categories, footer); content.setAlignment(Pos.TOP_CENTER); content.setPadding(new Insets(24, 24, 24, 24));
        ScrollPane scroll = new ScrollPane(content); scroll.setFitToWidth(true); scroll.setHbarPolicy(ScrollPane.ScrollBarPolicy.NEVER); scroll.setStyle("-fx-background: transparent; -fx-background-color: transparent; -fx-border-width: 0;");
        BorderPane page = new BorderPane(scroll); page.setTop(header(backAction, aboutAction));
    ...
    private BorderPane header(Runnable backAction, Runnable aboutAction) {
        ...
        header.setStyle("-fx-background-color: #f3e7ce; -fx-border-color: #d0c5af; -fx-border-width: 0 0 1px 0; -fx-effect: dropshadow(gaussian, rgba(58,48,39,0.10), 10, 0.28, 0, 1.5px);");
        return header;
    }

    private Button navButton(String text, boolean active) {
        Button button = new Button(text);
        String activeStyle = active ? "-fx-text-fill: #735c00; -fx-border-color: #735c00; -fx-border-width: 0 0 2px 0;" : "-fx-text-fill: #4d4635; -fx-border-color: transparent; -fx-border-width: 0 0 2px 0;";
        button.setStyle("-fx-background-color: transparent; -fx-background-radius: 0; -fx-font-size: 13px; -fx-font-weight: 700; -fx-font-family: 'Segoe UI', sans-serif; -fx-padding: 8px 4px; -fx-cursor: hand; " + activeStyle);
        return button;
    }
    ...
    private VBox createDesktopFooter() {
        ...
        Label promise = label("Connecting skilled workers with verified opportunities, fair work, and a stronger future.", "-fx-font-size: 13px; -fx-text-fill: #f8f0e2; -fx-opacity: 0.82;");
        ...
        Label copyright = label("© 2026 DIHADI  •  Mera Haq ~ Meri Dihadi. All rights reserved.", "-fx-font-size: 12px; -fx-text-fill: #f8f0e2; -fx-opacity: 0.65;");
        VBox footer = new VBox(24, footerMain, copyright);
        footer.setMaxWidth(1180);
        footer.setPadding(new Insets(32, 42, 24, 42));
        footer.setStyle("-fx-background-color: #343027; -fx-background-radius: 20px; -fx-border-color: rgba(208,197,175,0.32); -fx-border-radius: 20px; -fx-border-width: 1px 0 0 0;");
        return footer;
    }
    ...
    private Button footerLink(String text) {
        Button button = new Button(text);
        button.setStyle("-fx-background-color: transparent; -fx-padding: 2px 0; -fx-text-fill: #f8f0e2; -fx-opacity: 0.80; -fx-font-size: 13px; -fx-font-family: 'Segoe UI', sans-serif; -fx-cursor: hand;");
        return button;
    }
    ...
    private Button primaryButton(String text) { Button button = new Button(text); button.setStyle("-fx-background-color: #d4af37; -fx-background-radius: 999px; -fx-text-fill: #3a3027; -fx-font-size: 14px; -fx-font-weight: 700; -fx-font-family: 'Segoe UI', sans-serif; -fx-padding: 10px 20px;"); return button; }
    private Button outlineButton(String text) { Button button = new Button(text); button.setStyle("-fx-background-color: #fbf3e5; -fx-background-radius: 999px; -fx-border-color: #735c00; -fx-border-radius: 999px; -fx-text-fill: #735c00; -fx-font-size: 14px; -fx-font-weight: 700; -fx-font-family: 'Segoe UI', sans-serif; -fx-padding: 9px 18px;"); return button; }
    private ImageView image(String path, double width, double height) { ImageView image = new ImageView(loadImage(path)); if (path.contains("/assets/logo/")) { image.setViewport(new Rectangle2D(380, 0, 840, 840)); image.setPreserveRatio(true); } else { image.setPreserveRatio(false); } image.setFitWidth(width); image.setFitHeight(height); image.setSmooth(true); return image; }
    ...
    @Override public void start(Stage stage) { stage.setScene(getWorkerScene(null)); stage.setTitle("Dihadi - Worker Page"); stage.setWidth(1400); stage.setHeight(780); stage.setMaximized(true); stage.show(); }
}
